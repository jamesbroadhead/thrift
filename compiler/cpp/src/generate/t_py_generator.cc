  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    if (!is_oneway) {
      if (gen_twisted_) {
        ...
        f_service_ << indent() << "return future" << endl;
      } else {
        f_service_ << indent();
    ...
    if (gen_twisted_) {
      f_service_ <<
        indent() << "d.addCallbacks(" << endl;

      f_service_ <<
        indent() << indent() << "callback=self.cb_send_" << funname << "," << endl <<
        indent() << indent() << "callbackArgs=(seqid,)," << endl <<
        indent() << indent() << "errback=self.eb_send_" << funname << "," << endl <<
        indent() << indent() << "errbackArgs=(seqid,))" << endl;
      f_service_ <<
